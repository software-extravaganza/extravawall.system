...
namespace ExtravaWallSetup.Commands
{
    public class BasicCommands : CommandBase<BasicCommands>
    {
        ...
        public bool IsRunningElevated() {
            ...
            var isWindowsElevated = () => new WindowsPrincipal(WindowsIdentity.GetCurrent()).IsInRole(WindowsBuiltInRole.Administrator);
        ...
        public bool IsSupportedOs() {
            ...
            string pattern = @"(?<name>[^\.]+)\.(?<version>[^-]*)-?(?<architecture>[^-]*)-?(?<qualifiers>.*)";
            ...
            foreach (Match match in regex.Matches(runtimeIdentifier)) {
                if (match.Success) {
                    osName = (match.Groups.ContainsKey("name") ? match.Groups["name"].Value : string.Empty).ToLower();
                    osVersion = SemVersion.Parse(match.Groups.ContainsKey("version") ? match.Groups["version"].Value : string.Empty, SemVersionStyles.Any);
                    break;
                }
            }
