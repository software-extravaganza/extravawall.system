        ...
        .Executes(() => {
            DotNetTest(s => s
                ...
                .SetProcessArgumentConfigurator(arguments => arguments
                    ...
                    .Add("/p:AltCoverJson={0}", "artifacts/coverage/coverage.json")
    ...
    Target Docs => _ => _
        ...
        .Executes(async () => {
            var livingDocInstall = await Cli.Wrap("dotnet")
                .WithArguments(new[] { "tool", "install", "--global", "SpecFlow.Plus.LivingDoc.CLI" })
                .WithValidation(CommandResultValidation.None)
                .ExecuteAsync();
            if (livingDocInstall.ExitCode != 0 && livingDocInstall.ExitCode != 1) {
                throw new Exception($"Could not install SpecFlow.Plus.LivingDoc.CLI global dotnet tool in {nameof(Docs)} target.");
            }
            ...
            foreach (var dir in RootDirectory.GlobFiles("**/*.Test.csproj").Select(x => x.Parent).Distinct()) {
                ...
                .WithArguments(new[] { "feature-folder", featureFolder, "--output", testProjArtifactsDir / "tests/index.html", "-t", ArtifactsDirectory / "tests/TestResults.json" })
                .WithValidation(CommandResultValidation.None)
                .WithStandardErrorPipe(PipeTarget.ToStream(Console.OpenStandardError()))
                .ExecuteAsync();

                Console.WriteLine($"Living doc status for {featureFolder}: {(livingDocGenerate.ExitCode == 0 ? "success" : "fail")}");
                Console.WriteLine();
                Console.WriteLine();
            }
        });
...
public class TestResultConverter {
    ...
    public Root ConvertTrxToJson(string trxFilePath) {
        ...
        var root = new Root {
            ...
            ExecutionTime = DateTime.Parse(xdoc.Descendants(ns + "Times").First().Attribute("start").Value),
            GenerationTime = DateTime.Parse(xdoc.Descendants(ns + "Times").First().Attribute("finish").Value),
        ...
        foreach (var element in unitTestElements) {
            var executionResult = new ExecutionResult {
                ...
                StepResults = new List<StepResult> { new StepResult { Duration = "", Status = element.Attribute("outcome").Value, Error = "", Outputs = "" } }, // This needs to be replaced with correct mapping
